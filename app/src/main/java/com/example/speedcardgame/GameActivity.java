...
public class GameActivity extends AppCompatActivity implements View.OnClickListener, View.OnDragListener, View.OnTouchListener  {
    ...
    public boolean onDrag(View v, DragEvent event) {
        // manage with the stop of the drag
            ...
            case DragEvent.ACTION_DRAG_ENDED:
                ...
                if (cardPlayed != -1) {
                    ...
                    if (openNull1 == 5 && gameStrategy.pOneCards.isEmpty()) {
                        ...
                        editor.apply();
                        String spUserName = (sp.getString(getString(R.string.user_name), ""));
                        // insert xp and score to db
                        DBHelper dataManager = new DBHelper(this);
                        dataManager.getWritableDatabase();
                        Cursor cursor = dataManager.selectByName(spUserName);
                        if (cursor != null && cursor.getCount() > 0) {
                            int xp = 3000/(miliseconds/10);
                            if ((boolean)getIntent().getExtras().get("championship")) {
                                if ((int) getIntent().getExtras().get("phase") == 3)
                                    xp = xp + 350;// bonus winning a champ
                            }
                            dataManager.setXP(xp, cursor.getInt( cursor.getColumnIndex("_id") ));
                        ...
                        { //if playing championship:
                            builder.setPositiveButton("Continue Champ!", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    myHandler.removeCallbacksAndMessages(null);
                                    Intent intent = new Intent(GameActivity.this, ChampActivity.class);
                                    intent.putExtra("phase", ((int) getIntent().getExtras().get("phase")) + 1);
                                    startActivity(intent);
                                    finish();
                                    //recreate();
                                }
    ...
    private void runTimer() {
        ...
        myHandler.post(new Runnable() {
            ...
            public void run()
            {
                ...
                myHandler.postDelayed(this, 100);  // we are demanding to resume the Timer in 100 millisecs
            }
        });
    }
